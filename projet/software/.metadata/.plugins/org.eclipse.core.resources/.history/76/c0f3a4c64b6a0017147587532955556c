...
alt_u8 ConfigAudio(void){
	...
	if(audioConfigDevice != 0){
		...
		prinft("Audio configuration instantiated sucessfully!!!\r\n");
		...
		if(alt_up_av_config_write_audio_cfg_register(audioConfigDevice,LEFT_CHANNEL_OUT_REG,GAIN_0DB) != 0 ){
			printf("Left channel line-out couldn't be configured!!!\r\n");
			configFailure = 1;
		}
		//config du line-out droit
		if(alt_up_av_config_write_audio_cfg_register(audioConfigDevice,RIGHT_CHANNEL_OUT_REG,GAIN_0DB) != 0 ){
		...
		prinft("Audio configuration cannot be instantiated!!!\r\n");
		prinft("Please troubleshoot device instantiation or connection!!!\r\n");
		return 1;
	}
}
...
void SendAudioData(alt_u32 data, alt_up_audio_dev *audioDevice){
	...
	//alt_up_audio_write_fifo(audioDevice,data,AUDIO_DATA_SIZE,ALT_UP_AUDIO_RIGHTDATA_REG);
}
